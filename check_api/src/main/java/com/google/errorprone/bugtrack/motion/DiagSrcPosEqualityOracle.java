...
/**
 * When tracking diagnostics, we might want to track them via their line and column number or by
 * it's start position. Sadly the line/col information of a diagnostic is not guarenteed to map to a
 * specific start/end buffer location which means when trying to pipeline comparers we might have
 * some difficulty. To simplify this matter this class acts as a discriminated union for "position"
 * information for a diagnostic. An owner of an instance need not know whether it's tracking
 * line/col or src buffer but can merely query whether a diagnostic matches it's location.
 */
public class DiagSrcPosEqualityOracle implements DiagPosEqualityOracle {
  ...
  private DiagSrcPosEqualityOracle(
      EqualityType eqType, long line, long col, long startPos, long pos, long endPos) {
    this.eqType = eqType;
    this.line = line;
    this.col = col;
  ...
  public static DiagSrcPosEqualityOracle byStartAndEndPos(final long startPos, final long endPos) {
    return new DiagSrcPosEqualityOracle(
        EqualityType.BY_START_AND_END_POS, -1, -1, startPos, -1, endPos);
  }
  ...
  public long getStartPos() {
    if (!(eqType == EqualityType.BY_START_POS || eqType == EqualityType.BY_START_AND_END_POS)) {
      throw new RuntimeException(
          "cannot get start position if not tracking by start, and possibly end, pos");
    }
