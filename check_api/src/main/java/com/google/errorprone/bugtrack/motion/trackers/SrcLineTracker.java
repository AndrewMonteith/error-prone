    ...
    if (patch.isPresent()) {
      ...
      switch (patch.get().getType()) {
        ...
        case CHANGE:
          ...
          return offsetInChange == -1
              ? Optional.empty()
              : Optional.of((long) target.getPosition() + offsetInChange);
        default: // case DELETE:
          // If the line is deemed "deleted" from the old source file, then logically it's not in
          // the other
