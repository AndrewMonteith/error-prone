...
public class GitUtils {
  ...
  private static List<RevCommit> expandCommitRange(Repository repo, CommitRange range)
      throws GitAPIException, IOException, InterruptedException {
    Deque<RevCommit> commits = new LinkedList<>();

    // Can't be bothered to fall around with JGit
    List<String> gitLogLines =
        Splitter.on('\n')
            .splitToList(
                ShellUtils.runCommand(
                    ...
                    "--ancestry-path",
                    "--oneline",
                    range.startCommit + ".." + range.finalCommit));

    Set<String> partialIds =
        gitLogLines.stream().map(s -> s.split(" ")[0]).collect(ImmutableSet.toImmutableSet());
  ...
  public static List<DiffEntry> computeDiffs(
      Repository repo, RevCommit olderCommit, RevCommit newerCommit)
