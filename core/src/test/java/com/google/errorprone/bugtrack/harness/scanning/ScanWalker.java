...
public final class ScanWalker
    implements Iterable<Collection<DiagnosticsScan>>, Iterator<Collection<DiagnosticsScan>> {
  ...
  private void sanitiseFilesInScans(Collection<DiagnosticsScan> scans)
      throws IOException, InterruptedException {
    List<String> cmdToRun =
        scans.stream()
            .map(scan -> scan.files)
    ...
    ShellUtils.runCommand(project.getRoot(), cmdToRun.toArray(new String[0]));
  }
  ...
  public Collection<DiagnosticsScan> next() {
    try {
      ...
      ShellUtils.runCommand(
          project.getRoot(), ProjectFiles.find("error-prone", "sanitise_project.sh").toString());
